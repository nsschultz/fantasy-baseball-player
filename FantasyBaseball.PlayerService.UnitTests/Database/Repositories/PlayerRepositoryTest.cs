...
namespace FantasyBaseball.PlayerService.Database.Repositories.UnitTests
{
    public class PlayerRepositoryTest : IDisposable
    {
        ...
        [Fact]
        public async void GetPlayerByBhqIdNull() =>
            Assert.Null(await new PlayerRepository(_context).GetPlayerByBhqId(2, PlayerType.B));

        [Fact]
        public async void GetPlayerByBhqIdValid() =>
            Assert.Equal(2, (await new PlayerRepository(_context).GetPlayerByBhqId(2, PlayerType.P)).BhqId);
