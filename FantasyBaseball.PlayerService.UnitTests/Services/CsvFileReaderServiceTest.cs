...
namespace FantasyBaseball.PlayerService.Services.UnitTets
{
  ...
  {
    private static readonly List<string> BATTER_RESULTS =
    [
    ...
    private static readonly List<string> PITCHER_RESULTS =
    [
    ...
    [Fact]
    public async Task ReadCsvDataInvalidBatterTest()
    {
      var fileReader = new Mock<IFileReader>();
      fileReader.Setup(o => o.ReadLines()).ReturnsAsync(PITCHER_RESULTS);
      await Assert.ThrowsAsync<HeaderValidationException>(() => new CsvFileReaderService().ReadCsvData(new BhqBattingStatsMap(), fileReader.Object));
    }

    [Fact]
    public async Task ReadCsvDataValidBatterTest()
    {
      var fileReader = new Mock<IFileReader>();
      fileReader.Setup(o => o.ReadLines()).ReturnsAsync(BATTER_RESULTS);
      var results = await new CsvFileReaderService().ReadCsvData(new BhqBattingStatsMap(), fileReader.Object);
      Assert.Single(results);
      Assert.Equal(1234, results.First().BhqId);
    }

    [Fact]
    public async Task ReadCsvDataInvalidPitcherTest()
    {
      var fileReader = new Mock<IFileReader>();
      fileReader.Setup(o => o.ReadLines()).ReturnsAsync(BATTER_RESULTS);
      await Assert.ThrowsAsync<HeaderValidationException>(() => new CsvFileReaderService().ReadCsvData(new BhqPitchingStatsMap(), fileReader.Object));
    }

    [Fact]
    public async Task ReadCsvDataValidPitcherTest()
    {
      var fileReader = new Mock<IFileReader>();
      fileReader.Setup(o => o.ReadLines()).ReturnsAsync(PITCHER_RESULTS);
      var results = await new CsvFileReaderService().ReadCsvData(new BhqPitchingStatsMap(), fileReader.Object);
      Assert.Single(results);
      Assert.Equal(1234, results.First().BhqId);
    }
  }
