...
namespace FantasyBaseball.PlayerService.Services.UnitTests
{
    public class PlayerUpdateServiceTest : IDisposable
    {
        ...
        [Fact]
        public async void UpdatePlayerTestDatabaseException()
        {
            Assert.Equal("TB", _context.Players.Find(PlayerMatchingId).Team);
            var player = new BaseballPlayer { Id = PlayerMatchingId, BhqId = 2, Type = PlayerType.B, Team = new BaseballTeam { Code = "TB" } };
        ...
        {
            Assert.Equal("TB", _context.Players.Find(PlayerMatchingId).Team);
            var player = new BaseballPlayer { Id = PlayerMissingId, BhqId = 1, Type = PlayerType.B, Team = new BaseballTeam { Code = "MIL" } };
        ...
        {
            Assert.Equal("TB", _context.Players.Find(PlayerMatchingId).Team);
            var player = new BaseballPlayer { Id = PlayerMatchingId, BhqId = 1, Type = PlayerType.B, Team = new BaseballTeam { Code = "MIL" } };
            await new PlayerUpdateService(_context, BuildMergeService()).UpdatePlayer(player);
        ...
        private IPlayerEntityMergerService BuildMergeService()
        {
            ...
            mergeService.Setup(o => o.MergePlayerEntity(It.IsAny<BaseballPlayer>(), It.IsAny<PlayerEntity>()))
                .ReturnsAsync((BaseballPlayer player, PlayerEntity entity) =>
                    {
                        entity.BhqId = player.BhqId;
                        entity.Type = player.Type;
                        entity.Team = player.Team.Code;
                        return entity;
                    });
