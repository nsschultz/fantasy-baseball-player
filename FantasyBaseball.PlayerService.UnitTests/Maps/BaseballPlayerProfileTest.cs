...
public class BaseballPlayerProfileTest
{
  ...
  private static void ValidatePlayer(int value, PlayerEntity expected, BaseballPlayer actual)
  {
    ...
    Assert.Equal(value == 10 ? expected.LeagueStatuses[0].LeagueStatus : LeagueStatus.A, actual.League1);
    Assert.Equal(value != 10 ? expected.LeagueStatuses[0].LeagueStatus : LeagueStatus.A, actual.League2);
    ValidatePlayerBattingStats(value == 10 ? expected.BattingStats[0] : new BattingStatsEntity(), actual.BattingStats.ToDictionary(s => s.StatsType, s => s)[StatsType.YTD]);
    ValidatePlayerBattingStats(value != 10 ? expected.BattingStats[0] : new BattingStatsEntity(), actual.BattingStats.ToDictionary(s => s.StatsType, s => s)[StatsType.PROJ]);
    ValidatePlayerBattingStats(expected.BattingStats[0], actual.BattingStats.ToDictionary(s => s.StatsType, s => s)[StatsType.CMBD]);
    ValidatePlayerPitchingStats(value == 10 ? expected.PitchingStats[0] : new PitchingStatsEntity(), actual.PitchingStats.ToDictionary(s => s.StatsType, s => s)[StatsType.YTD]);
    ValidatePlayerPitchingStats(value != 10 ? expected.PitchingStats[0] : new PitchingStatsEntity(), actual.PitchingStats.ToDictionary(s => s.StatsType, s => s)[StatsType.PROJ]);
    ValidatePlayerPitchingStats(expected.PitchingStats[0], actual.PitchingStats.ToDictionary(s => s.StatsType, s => s)[StatsType.CMBD]);
  }
